...
describe('checkMissingFields', () => {
  test('returns empty array if all required fields are present', () => {
    const obj = { a: 1, b: 2 };
    expect(checkMissingFields(obj, ['a', 'b'])).toEqual([]);
  });
  test('returns array of missing fields', () => {
    const obj = { a: 1 };
    expect(checkMissingFields(obj, ['a', 'b', 'c'])).toEqual(['b', 'c']);
  });
  test('returns all fields if none are present', () => {
    const obj = {};
    expect(checkMissingFields(obj, ['x', 'y'])).toEqual(['x', 'y']);
  });
});

describe('checkInvalidTypes', () => {
  test('returns empty array if all fields are correct type', () => {
    const obj = { a: 'hello', b: 5 };
    expect(checkInvalidTypes(obj, { a: 'string', b: 'number' })).toEqual([]);
  });
  test('returns array of fields with wrong type', () => {
    const obj = { a: 1, b: 'not a number' };
    expect(checkInvalidTypes(obj, { a: 'string', b: 'number' })).toEqual([
      'a',
      'b',
    ]);
  });
  test('ignores fields that are undefined', () => {
    const obj = { a: 'hi' };
    expect(checkInvalidTypes(obj, { a: 'string', b: 'number' })).toEqual([]);
  });
});
